...
class LocalUpdate(object):
    ...
    def update_weights(self, model, model_head, training_round, epoch_num=1):
    ...
    def inference(self, model):
        ...
        print(f'Local Testing    | Accuracy: {np.round(accuracy, 4)} | Normal Acc: {np.round(acc_n, 4)}'
              f' | Anormal Acc: {np.round(acc_a, 4)} | Threshold: {np.round(best_threshold, 4)}')
